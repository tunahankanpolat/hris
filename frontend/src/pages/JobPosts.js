        ...
        <div className="w-1/2 h-full flex flex-col justify-between bg-white ">
          <div className="w-full h-full border shadaow pt-8 overflow-auto">
            {jobPosts.map((jobPost) => (
              <JobPostCard key={jobPost.jobPostId} {...jobPost}  onClick={() => setCurrentJobPost(jobPost)}/>
            ))}
          </div>
          <Pagination currentPage={page+1} totalPages={page+2} className="self-end" onPageChange={handlePageChange}/>
        </div>

        <div className="w-1/2 bg-white h-full border shadaow pt-8 overflow-auto">
          {currentJobPost && <JobPostDetail isApplied={false} {...currentJobPost} />}
        </div>
      </main>
    </div>
  );
